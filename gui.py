...
def check_answer(selected):
    ...
    if selected == correct_def:
        ...
        monster_hp -= damage
        score += 1
        message = f"✅ Correct! You hit the monster for {damage} damage."
    else:
        damage = random.randint(10, 20)
        player_hp -= damage
        message = f"❌ Wrong! The monster hit you for {damage} damage."

    score_label.config(text=f"Score: {score}")
    status_label.config(text=f"HP: You {player_hp} | Monster {monster_hp}")

    if monster_hp <= 0:
        messagebox.showinfo("Victory", f"You won! Final score: {score}")
        for btn in buttons:
            btn.config(state="disabled")
    elif player_hp <= 0:
        messagebox.showinfo("Defeat", f"You lost! Final score: {score}")
        for btn in buttons:
            btn.config(state="disabled")
    else:
        next_question()
